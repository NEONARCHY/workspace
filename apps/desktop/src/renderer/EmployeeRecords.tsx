...
export function EmployeeRecords({ employees, filterKey, selectedIds, onOpen, onToggle, onTogglePage }: {
    ...
    <div className="record-table-scroll" role="region" aria-label="Список сотрудников" tabIndex={0}>
      <table className="record-table employee-record-table" aria-label="Сотрудники">
        <thead><tr>
          ...
          {[["name", "Сотрудник"], ["position", "Должность"], ["username", "Логин"], ["role", "Роль"], ["status", "Состояние"]].map(([column, label]) => <SortHeading key={column} column={column!} sort={sort} onSort={onSort}>{label}</SortHeading>)}
        </tr></thead>
