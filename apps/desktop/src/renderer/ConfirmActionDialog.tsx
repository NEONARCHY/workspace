...
export function ConfirmActionDialog({ open, title, message, confirmLabel = "Удалить", busy = false, onCancel, onConfirm }: ConfirmActionDialogProps) {
    ...
    <DialogSurface className="confirm-action-dialog" aria-describedby="confirm-action-message">
      <DialogBody>
        ...
        <DialogActions>
          ...
          <Button className="confirm-action-danger" icon={<Delete24Regular />} disabled={busy} onClick={() => void onConfirm()}>{busy ? "Удаляем…" : confirmLabel}</Button>
        </DialogActions>
      </DialogBody>
    </DialogSurface>
  </Dialog>;
