...
function AbsenceCard({ item, name, onSelect }: { readonly item: AbsenceRequest; readonly name: string; readonly onSelect: () => void }) {
  return <button type="button" className="absence-card" onClick={onSelect}><span className={`absence-kind-mark is-${item.kind}`} /><span className="absence-card-main"><span><strong>{labels[item.kind]}</strong><small>{item.statusLabel}</small></span><p>{item.reason}</p><time>{new Date(item.startsAt).toLocaleString("ru-RU")} — {new Date(item.endsAt).toLocaleString("ru-RU")}</time><em>{name}</em>{item.documentStatus === "overdue" ? <b>Нужно заключение</b> : null}</span></button>;
}
