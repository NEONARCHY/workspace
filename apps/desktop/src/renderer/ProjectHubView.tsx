  ...
  return <section className="project-hub-view workspace-view" aria-label={mode === "projects" ? "Проекты" : "Проектные заявки"}>
    <header className="project-hub-header">
      ...
      <div className="project-hub-header-actions"><Button icon={<ArrowClockwise24Regular />} onClick={() => void reload()} disabled={loading}>Обновить</Button>
        ...
        {mode === "funding" && canCreateRequest ? <Button appearance="primary" icon={<Add24Regular />} onClick={() => { setFundingProjectId((id) => id || hub.projects[0]?.id || ""); setFundingWorkstreamId(""); setRequestForm({ itemId: "", title: "", purpose: "", amount: "", approvalDueAt: "" }); setActionError(""); setFormMode("request"); }}>Новая проектная заявка</Button> : null}</div>
    </header>
    ...
    <Dialog open={formMode !== null} onOpenChange={(_, data) => { if (!data.open && !busy) setFormMode(null); }}><DialogSurface className="project-hub-dialog" aria-labelledby="project-hub-dialog-title">
      ...
      {formMode === "request" ? <form onSubmit={(event) => { event.preventDefault(); const amount = Number(requestForm.amount); if (!requestProject || !requestForm.itemId) { setActionError("Выберите проект, направление и работу."); return; } if (!Number.isSafeInteger(amount) || amount <= 0) { setActionError("Введите положительную целую сумму."); return; } void mutate(async () => { const created = await createProjectHubRequest(token, requestProject.id, { itemId: requestForm.itemId, title: requestForm.title, purpose: requestForm.purpose, amount, approvalDueAt: new Date(requestForm.approvalDueAt).toISOString() }); setSelectedRequestId(created.id); }, true); }}><div className="project-hub-form-scroll"><p>Заявка пройдёт по маршруту выбранного проекта. После отправки к ней можно прикрепить файлы в этом разделе.</p><div className="project-hub-fields">{mode === "funding" ? <><label>Проект<WorkspaceSelect required value={fundingProjectId} onChange={(event) => { setFundingProjectId(event.target.value); setFundingWorkstreamId(""); setRequestForm({ ...requestForm, itemId: "" }); }}><option value="">Выберите проект</option>{hub.projects.filter((project) => project.lifecycleStatus === "active").map((project) => <option value={project.id} key={project.id}>{project.code} · {project.title}</option>)}</WorkspaceSelect></label><label>Направление<WorkspaceSelect required value={fundingWorkstreamId} onChange={(event) => { setFundingWorkstreamId(event.target.value); setRequestForm({ ...requestForm, itemId: "" }); }}><option value="">Выберите направление</option>{fundingWorkstreams.map((workstream) => <option value={workstream.id} key={workstream.id}>{workstream.title}</option>)}</WorkspaceSelect></label><label className="wide">Задача или мероприятие<WorkspaceSelect required value={requestForm.itemId} onChange={(event) => setRequestForm({ ...requestForm, itemId: event.target.value })}><option value="">Выберите работу</option>{fundingItems.filter((item) => item.status !== "cancelled").map((item) => <option value={item.id} key={item.id}>{item.kind === "task" ? "Задача" : "Мероприятие"} · {item.title}</option>)}</WorkspaceSelect></label></> : null}<label className="wide">Название<Input required value={requestForm.title} onChange={(_, data) => setRequestForm({ ...requestForm, title: data.value })} /></label><label>Сумма · {requestProject?.currency || "UZS"}<Input required type="number" min="1" value={requestForm.amount} onChange={(_, data) => setRequestForm({ ...requestForm, amount: data.value })} /></label><label>Крайний срок согласования<Input required type="datetime-local" value={requestForm.approvalDueAt} onChange={(_, data) => setRequestForm({ ...requestForm, approvalDueAt: data.value })} /></label><label className="wide">Назначение<Textarea value={requestForm.purpose} onChange={(_, data) => setRequestForm({ ...requestForm, purpose: data.value })} /></label></div><div className="project-hub-route-preview">{requestProject?.approverUserIds.length ? requestProject.approverUserIds.map((id, index) => <span key={id}>{index + 1}. {personName(id)}</span>) : "Маршрут не настроен: руководитель должен добавить согласующих."}</div></div><div className="project-hub-dialog-footer">{actionError ? <span role="alert">{actionError}</span> : null}<Button type="button" disabled={busy} onClick={() => setFormMode(null)}>Отмена</Button><Button type="submit" appearance="primary" disabled={busy || !requestProject?.approverUserIds.length || !requestForm.itemId}>Отправить на согласование</Button></div></form> : null}
