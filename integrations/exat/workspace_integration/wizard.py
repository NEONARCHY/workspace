...
class LetterWizard:
    ...
    def prompt(self, actor: str, context: dict[str, Any]) -> None:
        ...
        if step == "subject":
            ...
            if step == "preliminary":
                rows.append(
                    [opt("Только Бобур, без предварительного согласования", "preliminary", None)]
                )
            back = (
                "reviewer"
                if step == "preliminary"
    ...
    def callback(self, actor: str, data: str, operation: str) -> bool:
        ...
        if action == "go":
            ...
            context["step"] = "reviewer" if letter["workflowKind"] == "sign_only" else "attachments"
            ...
            if action == "reviewer":
                ...
                if value["key"] == "bobur" and letter["workflowKind"] == "delivery":
                    ...
                    self.submit(actor, context, str(uuid5(NAMESPACE_URL, operation + ":submit")))
                    return True
            else:
                bobur = next(
                    (r for r in reviewers if r["key"] == "bobur" and r["canApprove"]), None
                )
                if bobur is None or (value and value["key"] == "bobur"):
