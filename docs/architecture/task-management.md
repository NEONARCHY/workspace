...
- `DELETE /api/v1/tasks/{task_id}` с обязательной причиной

## Контролируемое удаление

Удалить задачу может её постановщик либо `admin`/`superadmin`. Сервер не стирает строку и связанную историю физически: миграция `0027_controlled_deletion` сохраняет автора, время и причину удаления, исключает задачу из рабочих выборок, отключает её цикл и убирает участников из автоматического чата задачи. Это сохраняет аудит и ссылки из исторических событий, но не оставляет удалённую задачу в интерфейсе.
